...
void hal_eeprom_flush(void) {
    ...
    hal_flash_write_buff(EEPROM_START_ADDR, (uint32_t *)eeprom_buf, sizeof(eeprom_buf)/8);
}
...
void hal_eeprom_init(void) {
    ...
    hal_flash_read_buf(EEPROM_START_ADDR, (uint32_t *)eeprom_buf, sizeof(eeprom_buf)/8);
...
uint8_t hal_get_flash_sector(uint32_t addr) {
    ...
    // if(addr<ADDR_FLASH_SECTOR_1)return FLASH_PAGE_0;
